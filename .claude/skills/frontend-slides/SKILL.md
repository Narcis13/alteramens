...
6. **Annotation Layer (MANDATORY)** — Every presentation MUST include a canvas drawing layer so the presenter can annotate slides live. No exceptions.
...
10. ✅ **Annotation layer is present** (canvas + toolbar + AnnotationLayer class)
...
## MANDATORY: Annotation Layer

**Every presentation generated by this skill MUST include the `AnnotationOverlay` from `JS_PRESETS.md`.** It is not optional and requires no additional HTML or CSS in the template — all DOM is built dynamically by the class itself.

### What it provides

- Tools: **free draw, line, arrow, rectangle, text, eraser**
- 7 preset colors + native color picker
- S / M / L stroke sizes
- Per-slide drawing state preserved automatically via MutationObserver
- Navigation lockout (wheel, touch, arrows) while a stroke is in progress
- Floating toggle button (pencil icon, bottom-left) + animated toolbox panel
- `E` to toggle on/off, `Escape` to close

### Required: `data-slide` attribute on every slide

`AnnotationOverlay` identifies slides by `slide.dataset.slide`. Every `<section class="slide">` **must** have a zero-indexed `data-slide` attribute:

```html
<section class="slide" id="slide-0" data-slide="0">...</section>
<section class="slide" id="slide-1" data-slide="1">...</section>
<!-- ...and so on for every slide -->
```

### No HTML or CSS needed

Unlike a custom implementation, `AnnotationOverlay` injects its own canvas and toolbox into `<body>` at runtime. Do **not** add any annotation-related HTML or CSS to the template.

### Initialization

Copy the `AnnotationOverlay` class verbatim from `JS_PRESETS.md → Annotation Overlay (Presenter Toolbox)`. Then initialize it **after** `SlidePresentation` in `DOMContentLoaded`, passing the theme's accent color as a hex string:

```javascript
document.addEventListener('DOMContentLoaded', () => {
    new SlidePresentation();
    new AnnotationOverlay({
        accentColor:  '#ACCENT_HEX',  // replace with this theme's accent color
        defaultColor: '#ffffff',       // starting pen color
    });
});
```

No `onSlideChange` hook is required in `SlidePresentation` — `AnnotationOverlay` uses its own MutationObserver to detect `.slide.visible` class changes.

### Keyboard hint update

Add `E · annotate` to the keyboard hint element:

```html
<span class="key">E</span>
<span style="margin-left:3px">annotate</span>
```

### Annotation Layer Checklist

Before delivering any presentation, verify:

1. ✅ Every `.slide` has `data-slide="N"` (0-indexed)
2. ✅ `AnnotationOverlay` class copied verbatim from `JS_PRESETS.md`
3. ✅ `new AnnotationOverlay({ accentColor: '<theme hex>' })` called after `SlidePresentation`
4. ✅ No annotation HTML or CSS added to template (class builds its own DOM)
5. ✅ Keyboard hint mentions `E` for annotate
...
**Always use presets from `JS_PRESETS.md` — never regenerate JS from scratch.**

Every presentation requires:

1. **Core** — Copy `SlidePresentation` class verbatim from `JS_PRESETS.md → CORE`. It handles keyboard, wheel, touch, progress bar, nav dots, and `.visible` animation triggers. **The `initObserver` method MUST call `window.annotationLayer?.onSlideChange(idx)` when the active slide changes.**

2. **Annotation Layer (MANDATORY)** — Copy the `AnnotationOverlay` class verbatim from `JS_PRESETS.md → Annotation Overlay (Presenter Toolbox)`. Initialize it after `SlidePresentation`. Never omit this. See the **MANDATORY: Annotation Layer** section above for exact integration steps.

3. **Optional Enhancements** — Pick from `JS_PRESETS.md → OPTIONAL ENHANCEMENTS` based on style. Use the style/enhancement mapping table in that file to decide which to include.
...
- Dots on the right to jump to any slide

Annotation (draw on slides live):
- E — toggle annotation mode (pencil button bottom-left)
- Escape — close annotation mode
- Tools: free draw, line, arrow, rectangle, text, eraser
- 7 preset colors + color picker, S/M/L stroke sizes
- Clear button inside the toolbox

To customize:
- Colors: edit :root CSS variables at the top
- Fonts: change the Google/Fontshare link
- Animations: modify .reveal timings
